...
const GrupoList = ({ grupos, onGuardar, hospitales = [] }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!id_hospital) {
        try {
          const hospitalResponse = await fetch("http://localhost:4000/api/superadmin/hospitals")
      ...
      const response = await fetch(`http://localhost:4000/api/groups/update-groups`, {
        method: "PUT",
        headers: {
  ...
  const handleConfirmarEliminar = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/api/groups/delete-groups/${grupoEliminar.id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const gruposFiltrados = grupos.filter((grupo) => {
    const coincideBusqueda =
      ...
      grupo.descripcion_group?.toLowerCase().includes(busquedaGrupo.toLowerCase())
          ...
          {Object.entries(gruposPorEstado).map(([estado, municipios]) => (
            <div key={estado} className="mb-8">
              {Object.entries(municipios).map(([municipio, hospitales]) => (
                <div key={municipio} className="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-6">
                          ...
                          <tbody className="bg-white divide-y divide-gray-100">
                            {gruposHospital.map((grupo) => (
                              <tr key={grupo.id_group} className="hover:bg-gray-50">
                                <td className="w-1/3 px-6 py-3 text-sm whitespace-normal">{grupo.nombre_grupo}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={handleCerrarModal} className="text-gray-400 hover:text-gray-600 transition-colors">
                <X className="h-6 w-6" />
              </button>
            </div>
