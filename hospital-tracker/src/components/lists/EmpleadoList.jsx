    ...
    <div className="bg-white shadow-md rounded-xl overflow-hidden">
      ...
      {(() => {
        ...
        const empleadosPorEstado = empleadosFiltrados.reduce((acc, empleado) => {
          ...
          if (!acc[estado][municipio]) acc[estado][municipio] = {}

          const hospital = empleado.hospital || "Sin hospital"
          if (!acc[estado][municipio][hospital]) acc[estado][municipio][hospital] = []

          acc[estado][municipio][hospital].push(empleado)
          return acc
        }, {})
                  ...
                  {Object.entries(municipios).map(([municipio, hospitales]) => (
                    <div key={`${estado}-${municipio}`} className="bg-white p-3 rounded-lg border border-slate-300">
                      ...
                      <div className="space-y-3">
                        {Object.entries(hospitales).map(([hospital, empleadosHospital]) => {
                          const empleadosVisibles = mostrarTodosEmpleados[`${estado}-${municipio}-${hospital}`]
                            ? empleadosHospital
                            : empleadosHospital.slice(0, 5)

                          return (
                            <div
                              key={`${estado}-${municipio}-${hospital}`}
                            ...
                            >
                              <div className="bg-slate-100 px-4 py-2 rounded-t-lg">
                                <h6 className="text-sm font-medium text-slate-700 flex items-center">
                                  ...
                                  <span className="ml-2 text-xs bg-amber-100 text-amber-700 px-2 py-1 rounded-full">
                                    {empleadosHospital.length} empleado
                                    {empleadosHospital.length !== 1 ? "s" : ""}
                                  </span>
                                </h6>
                              </div>
                                  ...
                                  <tbody className="bg-white divide-y divide-slate-100">
                                    {empleadosVisibles.map((empleado, index) => (
                                      <tr key={empleado.id_user || index} className="hover:bg-slate-50">
                                        <td className="px-4 py-3 whitespace-nowrap text-sm">{empleado.nombre}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm">{empleado.ap_paterno}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm">{empleado.ap_materno}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-mono text-xs">
                                          {empleado.curp_user || empleado.curp}
                                        </td>
                              ...
                              {empleadosHospital.length > 5 && (
                                <div className="px-4 py-3 bg-slate-50 border-t border-slate-200 text-center">
                                  <button
                                    onClick={() => {
                                      const key = `${estado}-${municipio}-${hospital}`
                                      setMostrarTodosEmpleados({
                                        ...mostrarTodosEmpleados,
                                        [key]: !mostrarTodosEmpleados[key],
                                      })
                                    }}
                                    className="text-amber-600 hover:text-amber-800 text-sm font-medium"
                                  >
                                    {mostrarTodosEmpleados[`${estado}-${municipio}-${hospital}`]
                                      ? "Mostrar menos"
                                      : `Ver todos (${empleadosHospital.length})`}
                                  </button>
                                </div>
                              )}
                            </div>
