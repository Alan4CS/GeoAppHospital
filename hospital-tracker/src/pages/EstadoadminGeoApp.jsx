...
export default function EstadoAdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const hospitalesResponse = await fetch(`http://localhost:4000/api/estadoadmin/hospitals-by-user/${userId}`)
        ...
        const empleadosSimulados = [
          ...
          },
          {
            ...
            puesto: "Técnica",
            hospital: hospitalesData[2]?.nombre_hospital || "Hospital General de Chetumal",
        ...
        const municipiosConEmpleados = [...new Set(empleadosSimulados.map((emp) => emp.municipio))].filter(Boolean)
  ...
  // Calcular KPIs
  const conectadosCount = empleados.filter((emp) => emp.status === "connected").length
  const desconectadosCount = empleados.filter((emp) => emp.status === "disconnected").length
  const fueraGeocercaCount = empleados.filter((emp) => emp.outsideGeofence).length
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (detailsRef.current && !detailsRef.current.contains(event.target) && window.innerWidth < 768) {
      ...
      <div className={sidebarClass}>
        <div className="p-4 flex items-center justify-between">
          <div className={`flex items-center ${!sidebarOpen && "justify-center w-full"}`}>
            ...
            <h1 className={`ml-2 font-bold text-xl ${!sidebarOpen && "hidden"}`}>Geo App</h1>
          </div>
          <button
          ...
          <div className={`flex items-center ${!sidebarOpen ? "flex-col" : ""}`}>
            ...
            {sidebarOpen && (
              <div className="ml-3">
                ...
                <p className="text-xs text-blue-200">{estadoActual || "Cargando..."}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col flex-1">
          <button onClick={() => setActiveTab("monitoreo")} className={sidebarButtonClass}>
        ...
        <main className="p-6">
          {isLoading ? (
            ...
            <>
              ...
              {activeTab === "monitoreo" && (
                <div
                  className={`relative bg-white rounded-xl shadow-md overflow-hidden ${selectedEmployee && detailsPosition === "bottom" ? "pb-96" : ""}`}
                >
                  <div className="flex h-[calc(100vh-250px)]">
                    ...
                    >
                      <div className="p-4">
                          ...
                          >
                            ...
                            {municipios.map((municipio) => (
                              <option key={municipio} value={municipio}>
                                {municipio} ({empleados.filter((emp) => emp.municipio === municipio).length})
                              </option>
                            ))}
                          </select>
                        </div>
                        ...
                        {selectedMunicipio && (
                          <div className="mb-4">
                            ...
                            >
                              ...
                              {hospitales
                                .filter((h) => h.municipio === selectedMunicipio)
                                .filter((h) => empleados.some((emp) => emp.hospital === h.nombre_hospital))
                                .map((hospital) => (
                                  <option key={hospital.id_hospital} value={hospital.nombre_hospital}>
                                    {hospital.nombre_hospital} (
                                    {empleados.filter((emp) => emp.hospital === hospital.nombre_hospital).length})
                                  </option>
                                ))}
                            </select>
                          </div>
                        ...
                        <div className="mt-6">
                          <h4 className="font-medium text-sm text-gray-700 mb-2">Empleados conectados</h4>
                              ...
                              .map((emp) => (
                                <div
                                  ...
                                  className={`p-2 rounded-md cursor-pointer flex items-center ${
                                    selectedEmployee === emp.id ? "bg-blue-100" : "hover:bg-gray-100"
                                  }`}
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-gray-500 truncate">{emp.hospital}</p>
                                  </div>
                                  {emp.outsideGeofence && (
                    ...
                    <div
                      className={`flex-1 relative ${selectedEmployee && detailsPosition === "right" ? "pr-80" : ""}`}
                    >
                      ...
                      >
                        {showFilters ? <ChevronLeft className="h-4 w-4" /> : <Filter className="h-4 w-4" />}
                      </button>
                              ...
                              >
                                <Popup>
                                  <div className="text-sm">
                                    ...
                                    <p className="text-gray-600">{employee.hospital}</p>
                                    <div className="mt-2 flex items-center">
                                      <Clock className="mr-1 text-gray-500 h-4 w-4" />
                                      <span>{employee.horasConectado.toFixed(1)} horas trabajadas</span>
                                    </div>
                                    {employee.outsideGeofence && (
                                      <div className="mt-1 text-orange-600 flex items-center">
                                        <AlertTriangle className="mr-1 h-4 w-4" />
                                        <span>Fuera de geocerca ({employee.salidasGeocerca} salidas)</span>
                                      </div>
                                    )}
                                    <button
                                    ...
                                    >
                                      <Info className="mr-1 h-3 w-3" /> Ver detalles completos
                                    </button>
                                  </div>
                                </Popup>
                              </Marker>
                            ...
                            return (
                              <div className="p-4">
                                <div className="flex justify-between items-start mb-4">
                                  <h3 className="font-medium text-lg">Detalles del Empleado</h3>
                                  <button
                                ...
                                {detailsPosition === "bottom" ? (
                                  ...
                                  <div className="flex flex-col md:flex-row md:space-x-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex-1">
                                      <div className="bg-gray-50 p-3 rounded-lg">
                                        <h5 className="text-sm font-medium text-gray-500 mb-1">Estado</h5>
                                        <div
                                          className={`flex items-center ${
                                            employee.status === "connected" ? "text-green-600" : "text-gray-600"
                                          }`}
                                        >
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">
                                          {format(employee.ultimaConexion, "HH:mm", { locale: es })}
                                        </p>
                                      </div>

                                      <div className="bg-gray-50 p-3 rounded-lg">
                                        <h5 className="text-sm font-medium text-gray-500 mb-1">Tiempo</h5>
                                        <div className="flex items-center">
                                          <Clock className="mr-1 text-gray-600 h-4 w-4" />
                                          <span>{employee.horasConectado.toFixed(1)} horas</span>
                                        </div>
                                      </div>

                                      <div
                                        className={`p-3 rounded-lg ${
                                          employee.outsideGeofence ? "bg-orange-50" : "bg-green-50"
                                        }`}
                                      >
                                        <h5 className="text-sm font-medium text-gray-500 mb-1">Geocerca</h5>
                                        {employee.outsideGeofence ? (
                                          <div className="text-orange-600 flex items-center">
                                            <AlertTriangle className="mr-1 h-4 w-4" />
                                            <span>Fuera ({employee.salidasGeocerca})</span>
                                          </div>
                                        ) : (
                                    ...
                                    <div className="mt-4 md:mt-0">
                                      <h5 className="text-sm font-medium text-gray-500 mb-2">Historial de actividad</h5>
                                      <div className="flex space-x-4">
                                        <div className="flex flex-col items-center">
                                          ...
                                          <div className="text-xs mt-1">
                                            {format(
                                              new Date(
                                                employee.ultimaConexion.getTime() - employee.horasConectado * 3600000,
                                              ),
                                              "HH:mm",
                                              { locale: es },
                                            )}
                                        ...
                                        {employee.salidasGeocerca > 0 && (
                                          <div className="flex flex-col items-center">
                                            ...
                                            <div className="text-xs mt-1">
                                              {format(new Date(employee.ultimaConexion.getTime() - 5400000), "HH:mm", {
                                                locale: es,
                                              })}
                                            </div>
                                        ...
                                        <div className="flex flex-col items-center">
                                          ...
                                          <div className="text-xs text-gray-500">
                                            {employee.status === "connected" ? "Actual" : "Fin"}
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  ...
                                  <>
                                    ...
                                    <div className="space-y-4">
                                      <div className="bg-gray-50 p-3 rounded-lg">
                                        <h5 className="text-sm font-medium text-gray-500 mb-1">Ubicación</h5>
                                        <p className="text-gray-800">{employee.hospital}</p>
                                        <p className="text-gray-600 text-sm">
                                          {employee.municipio}, {employee.estado}
                                        </p>
                                      </div>
                                      ...
                                      <div className="bg-gray-50 p-3 rounded-lg">
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">
                                          Última conexión:{" "}
                                          {format(employee.ultimaConexion, "d 'de' MMMM, HH:mm", { locale: es })}
                                        </p>
                                      </div>

                                      <div className="bg-gray-50 p-3 rounded-lg">
                                        <h5 className="text-sm font-medium text-gray-500 mb-1">Tiempo de trabajo</h5>
                                        <div className="flex items-center">
                                          <Clock className="mr-1 text-gray-600 h-4 w-4" />
                                          <span>{employee.horasConectado.toFixed(1)} horas</span>
                                        </div>
                                      ...
                                      >
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">
                                          Salidas registradas: {employee.salidasGeocerca}
                                        </p>
                                      </div>
                                      ...
                                      <div className="pt-2">
                                        ...
                                        <div className="space-y-2 text-sm">
                                          ...
                                          {employee.salidasGeocerca > 0 && (
                                            <div className="flex justify-between pb-2 border-b border-gray-100">
                                              <span className="text-gray-600">Primera salida</span>
                                              <span className="text-gray-800">
                                                {format(
                                                  new Date(employee.ultimaConexion.getTime() - 5400000),
                                                  "HH:mm",
                                                  {
                                                    locale: es,
                                                  },
                                          ...
                                          {employee.status === "connected" ? (
                                            ...
                                            <div className="flex justify-between pb-2 border-b border-gray-100">
                                              <span className="text-gray-600">Desconexión</span>
                                              <span className="text-gray-800">
                                                {format(employee.ultimaConexion, "HH:mm", { locale: es })}
                                              </span>
                                            </div>
              ...
              {activeTab === "hospitales" && (
                <div className="bg-white shadow-md rounded-xl overflow-hidden">
                  <div className="p-6 border-b border-gray-200">
                    <div className="flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0">
                      ...
                      <div className="relative">
                        <input type="search" placeholder="Buscar hospital..." className="px-4 py-2 border rounded-lg" />
                      </div>
                    </div>
                  </div>
                  ...
                  {hospitales.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full table-fixed text-sm">
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {hospitales.map((h, i) => (
                            <tr key={h.id_hospital || i} className="hover:bg-gray-50">
                              <td className="px-6 py-4 truncate">{h.nombre_hospital}</td>
                              <td className="px-6 py-4">{h.tipo_hospital}</td>
                              <td className="px-6 py-4">{h.municipio}</td>
                              <td className="px-6 py-4 truncate">{h.direccion_hospital}</td>
                              <td className="px-6 py-4">{h.radio_geo ?? 0} m</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="p-6 text-center text-gray-500">No hay hospitales disponibles.</div>
                  )}
                </div>
              )}
            </>
